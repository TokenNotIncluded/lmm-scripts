#!/usr/bin/env bash
set -euo pipefail
[[ $# = 0 ]] || { echo 'Usage: bash clash-verge-rev.sh' >&2; exit 2; }
if [[ -n ${TERMUX_VERSION:-} || ${PREFIX:-} == */com.termux/files/usr ]]; then
  echo 'Desktop applications are not supported in Termux.' >&2; exit 1
fi
dir=$(dirname -- "${BASH_SOURCE[0]:-}")
if [[ -n ${BASH_SOURCE[0]:-} && -f $dir/desktop.sh ]]; then exec bash "$dir/desktop.sh" clash-verge-rev; fi
